...
class loghandler():
    ...
    def init(self):
        ...
        if not Path(os.path.join(self.root_path, f'app/data/{self.file_name}')):
            log_file = open(os.path.join(self.root_path, 'app/data/', self.file_name), encoding='utf-8')  # Create file
            log_file.close()
    ...
    def write_log(self, text: str):
        ...
        log_file = open(os.path.join(self.root_path, 'app/data/', self.file_name), 'a', encoding='utf-8')  # Open file
        log = time + ' | ' + text
        log_file.write(log + '\n')
        log_file.close()
        return log
