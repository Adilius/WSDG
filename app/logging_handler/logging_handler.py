...
def setup_logging(logfile_file, log_level):
    ...
    log_line_string_format = '%(color_on)s%(asctime)s %(levelname)s: %(message)s%(color_off)s'
    log_line_date_format = "%Y-%m-%d %H:%M:%S"

    # Console handler
    logger.setLevel(log_level)  # Set console log level
    console_handler = logging.StreamHandler(sys.stdout) # Setup StreamHandler (Prints to console)
    console_formatter = LogFormatter(fmt=log_line_string_format, datefmt=log_line_date_format, color=True)  # Create formatter
    ...
    if log_level in ["DEBUGV", "DEBUG"]:
        logfile_handler.setLevel("INFO")    # Never save logging level debug and below log to file
    else:
        logfile_handler.setLevel(log_level)
    logfile_formatter = LogFormatter(fmt=log_line_string_format, datefmt=log_line_date_format, color=False) # Create formatter
