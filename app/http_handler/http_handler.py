...
def grab_user_id(session):
    ...
    try:
        ...
        logging.critical("Failed to get authentication token! Exiting program")
        sys.exit(1)
    else:
...
def log_handle_response(response, supply_drop_type: str):
    ...
    if response.status_code == 403:
        logging.debug(
            f"No {supply_drop_type} supply drop. Status code: {response.status_code}"
        )
        return response
    ...
    if response.status_code != 200:
        logging.error(
            f"Error grabbing {supply_drop_type} supply drop. Status code: {response.status_code}"
        )
        return response
    ...
    try:
        ...
        for drop in response_json["drops"]:
            ...
            logging.info(
                f"Grabbed {supply_drop_type} supply drop {name}{' ' if description == '' else f' and got {description}'}"
            )
    except:
        # Response parsing failed
        logging.info(
            f"Grabbed {supply_drop_type} supply drop. But failed to parse response text. Logging response for debugging"
        )
